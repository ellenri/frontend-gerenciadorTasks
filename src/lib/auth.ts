 ...
 */
export async function requireSession(Astro: APIContext): Promise<Session> {
  const session = await getSession(Astro);
  if (!session) throw Astro.redirect('/login');
  return session;
}
 ...
 */
export async function requireParent(Astro: APIContext): Promise<Session> {
  const session = await requireSession(Astro);
  if (session.user.role !== 'Parent') throw Astro.redirect(homeForRole(session.user.role));
  return session;
}
 ...
 */
export async function requireChild(Astro: APIContext): Promise<Session> {
  const session = await requireSession(Astro);
  if (session.user.role !== 'Child') throw Astro.redirect(homeForRole(session.user.role));
  return session;
}
